...
static uint32_t DEFAULT_DISPLAY_ID = ISurfaceComposer::eDisplayIdMain;	/* enum {
																					eDisplayIdMain = 0,
																					eDisplayIdHdmi = 1
																				};*/
...
int main(int argc, char** argv)
{
    ...
    ProcessState::self()->setThreadPoolMaxThreadCount(0);//+ ProcessState::self()获取对象指针
    ...
    int c;
    while ((c = getopt(argc, argv, "phd:")) != -1) {	//获取输入命令信息
        switch (c) {
            case 'p':
                png = true;
                break;
            case 'd':
                displayId = atoi(optarg);	//atoi字符串转整型
                break;
            case '?':
            case 'h':
                usage(pname);
    ...
    argc -= optind;//结束while循环后，查找其它参数，
	argv += optind;	// 为了跳过已经读取的参数，需要argc -= optind;args += optind;
